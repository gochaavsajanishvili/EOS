                    ...
                    this.data = {
                        ...
                        departmentalScorecards: {
                            technology: [
                                ...
                                { metric: "Bugs Closed Weekly", goal: "15", value: "18", trend: [14, 16, 17, 18] },
                                { metric: "Code Coverage %", goal: "80", value: "82", trend: [75, 78, 80, 82] }
                            ],
                            marketing: [
                                ...
                                { metric: "Keyword Rankings Top 3", goal: "20", value: "18", trend: [15, 16, 17, 18] },
                            ...
                            operations: [
                                ...
                                { metric: "Machines Shipped Weekly", goal: "10", value: "12", trend: [9, 10, 11, 12] },
                                { metric: "Sales Qualified Leads", goal: "30", value: "35", trend: [28, 32, 33, 35] },
                                { metric: "Order Fulfillment %", goal: "95", value: "94", trend: [92, 93, 93, 94] }
                            ]
                        },
                        scorecardHistory: {},
                        companyRocks: [
                        ...
                        meetingHistory: [
                            { id: "mtg1", date: "Sep 22, 2025", department: "technology", rating: 9, sentiment: 85, keyTopics: ["Shopify Sync", "API Performance"], completedTodos: [504], message: "Tech team will provide update on sync fix.", transcriptionUrl: "https://meet.google.com/recordings/tech-092225", notes: "Discussed Odoo sync. Prince fixed mobile login. Team morale high." },
                            ...
                            { id: "mtg3", date: "Sep 22, 2025", department: "operations", rating: 7, sentiment: 75, keyTopics: ["Repair Times", "Parts"], completedTodos: [503], message: "Maria contacted supplier.", transcriptionUrl: "https://meet.google.com/recordings/ops-092225", notes: "Pump shortage critical. Repair times above target." },
                ...
                document.body.addEventListener('click', (e) => {
                    ...
                    if (target.id === 'sync-meet-btn' || target.classList.contains('google-meet-sync')) {
                        ...
                        setTimeout(() => {
                            ...
                            if (App.data.meetingHistory.length > 0) {
                                App.data.meetingHistory[0].hasTranscript = true;
                                App.data.meetingHistory[0].hasRecording = true;
                                Persistence.save(App.data);
                            }
                        }, 2000);
                    }
                    ...
                    if (target.classList.contains('customer-to-ids') || target.classList.contains('employee-to-ids')) {
                        ...
                        if (row) {
                            ...
                            const headline = inputs[1]?.value || inputs[0]?.value || 'Issue from headlines';
                            if (headline) {
                                const newIssue = {
                                    id: Date.now(),
                                    title: headline,
            ...
            addVtoValue(field) {
                ...
                if (value) {
                    ...
                    if (fields.length === 1) {
                        ...
                        App.data.vto[fields[0]][fields[1]].push(value);
                    }
                    Persistence.save(App.data);
                    App.router();
                }
...
Alright, captain “no experiments.” Here’s your fully instantiated, copy-pasteable prompt built from your anatomy and theme, locked to the universal stack. It tells the model exactly what to scaffold, how to normalize the theme, and which packs to include. No Chart.js, only Recharts. Go build.
...
* Framework: **[next@15.5.x](mailto:next@15.5.x)**
* React: **[react@19.1.x](mailto:react@19.1.x)**, **[react-dom@19.1.x](mailto:react-dom@19.1.x)**
* Language: **[typescript@5.6.x](mailto:typescript@5.6.x)**
* CSS: **[tailwindcss@3.4.x](mailto:tailwindcss@3.4.x)**, **[postcss@8.5.x](mailto:postcss@8.5.x)**, **[autoprefixer@10.4.x](mailto:autoprefixer@10.4.x)**
* Lint/format: **[eslint@9.x](mailto:eslint@9.x)**, **[eslint-config-next@15.5.x](mailto:eslint-config-next@15.5.x)**, **[prettier@3.x](mailto:prettier@3.x)**
* Icons: **lucide-react@0.544.0**
* Forms & validation: **[react-hook-form@7.62.x](mailto:react-hook-form@7.62.x)**, **@hookform/resolvers@5.2.x**, **[zod@3.23.x](mailto:zod@3.23.x)**
* RPC: **@trpc/server@11.x**, **@trpc/react-[query@11.x](mailto:query@11.x)**
* Client data fetching & cache: **@tanstack/react-[query@5.x](mailto:query@5.x)**
* Env safety: **[envsafe@3.x](mailto:envsafe@3.x)** with Zod
* Testing: **[vitest@2.x](mailto:vitest@2.x)**, **@testing-library/react@16.x**, **@testing-library/jest-[dom@6.x](mailto:dom@6.x)**, **[msw@2.x](mailto:msw@2.x)**
* E2E: **@playwright/test@1.48.x**
* Observability: **@sentry/nextjs@8.x**
* Charts: **[recharts@3.1.x](mailto:recharts@3.1.x)**
...
Paste that into your AI tool and let it do the scaffolding. If it tries to sneak in Chart.js, dock its pay.

- Use Next.js 15 (App Router) + TypeScript + Tailwind CSS v3.4.x (no CDN).
- Package manager: pnpm.
- Icons: lucide-react (stable).
- State/data: React Query.
- Server API: tRPC (procedures typed end-to-end).
- Auth: Auth.js with email/password placeholder provider (no social required now).
- DB: PostgreSQL (RDS). ORM: Prisma. Provide prisma/schema.prisma + seed.ts.
- Charts: Recharts (replace any Chart.js).
- Forms/validation: react-hook-form + zod.
- RBAC: roles {admin, manager, member, viewer} enforced on server procedures.
- Realtime: SSE endpoint for status/updates; header indicator reflects connection.
- No experimental flags. No external CDN scripts. No inline API keys.
- Tailwind theme per our Dark/Indigo guide (map tokens in tailwind.config.js).
- 100% compatibility, simplicity, and free/open tooling only.

(Deliverables)
- Full Next.js app file tree (paths + file contents).
- .env.example, prisma/schema.prisma, prisma/seed.ts.
- Mock mode: `USE_MOCK=1` runs with in-memory data; same UI.
- Commands: `pnpm install`, `pnpm dev`.
- Acceptance tests: (1) runs in mock mode; (2) Add/Edit Rock works with optimistic update; (3) Recharts render; (4) SSE dot toggles; (5) no console errors.

=== HTML_TO_NEXT_CONVERSION_RULES ===
Short answer: it’ll get you very close, but “perfect” only happens if you tell the LLM exactly how to map your one-file SPA into the Next.js App Router, how to replace Chart.js, how to move Tailwind-from-CDN into a build, and how to de-DOM-ify all those `document.getElementById` handlers. Add the block below to the top of the prompt I gave you and you’ll have a reliable, deterministic conversion + upgrade path.
...
If you want, I can also spit out the exact Prisma schema + a minimal `HashRouter.tsx` and a Recharts `Line.tsx` wrapper so you can drop them in immediately.

Routing
- Keep hash routes (#dashboard, #vto, #departments, #departments/{id}, #people, #data, #issues, #rocks, #l10, #integrations).
- Implement `src/components/layout/HashRouter.tsx` that reads `window.location.hash` and renders feature components.
- Sidebar links set `location.hash` and highlight active.

Layout & theming
- Break source HTML into:
  - `src/components/layout/Sidebar.tsx`
  - `src/components/layout/HeaderBar.tsx`
  - `src/components/layout/Modal.tsx` (portal + focus trap)
  - `src/app/layout.tsx` uses `next/font/google` Inter; applies Tailwind dark theme.
- Move inline styles to Tailwind utilities or `src/styles/globals.css`.
- Map Theme Guide colors into `tailwind.config.js` (extend.colors).

Icons
- Replace inline SVG strings with lucide-react (e.g., Home, Target, Users, BarChart3, Bug, Mountain, Video, PlugZap, Bot).

State & data
- Replace global `App` and DOM manipulation with React state + React Query.
- Seed all demo data to Prisma models (User, Department, Employee, Rock, Issue, Todo, ScorecardMetric, Meeting, etc.).
- Provide a mock in-memory store when `USE_MOCK=1`.

Forms & modals
- Convert forms (Rocks, Employees, Metrics) to react-hook-form + zod schemas.
- Use reusable `Modal` component; no `innerHTML`.

Charts
- Remove Chart.js. Rebuild:
  - Dashboard sales trend => Recharts `<LineChart/>` in a responsive container.
  - Scorecard sparklines => mini `<LineChart/>` without axes/legend.

Realtime
- `src/app/api/sse/route.ts` streams `heartbeat` and simulated `metric:update`/`rock:update`/`issue:update` in mock mode.
- Header shows live/paused indicator.

AI assistant
- Remove hardcoded Gemini key. Add `src/app/api/ai/route.ts` that proxies to `process.env.AI_PROVIDER` with `AI_API_KEY`. Client uses typed fetch; sanitize content; no `dangerouslySetInnerHTML`.

Security
- No direct DOM `document.getElementById` listeners; use React handlers.
- Avoid `dangerouslySetInnerHTML`; if unavoidable, sanitize.
- Server-side RBAC guards on tRPC procedures.

Config
- `.env.example` with `DATABASE_URL`, `AUTH_SECRET`, `AI_PROVIDER`, `AI_API_KEY`.
- `tailwind.config.js` with dark palette, Inter via `next/font/google`.

Acceptance checks (must pass)
- `pnpm dev` boots with mock data; all hash pages navigate.
- Create/Edit Rock re-renders without refresh (optimistic update).
- No Chart.js in bundle; Recharts renders.
- SSE dot flips when endpoint stops.
- No console errors.

(Output format)
- Print the **full repo tree** then **each file’s content** in code fences, ready to paste.
```

Tips:

* Keep the three sections exactly as shown; the second and third blocks are your “universal prompt” and “conversion rules.”
* Paste your actual HTML inside `=== SOURCE_HTML ===`.
* If you tweak the theme or data models later, edit only the **UNIVERSAL_FOUNDATION** block; the conversion rules stay stable.
